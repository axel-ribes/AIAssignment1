...
Graph FileReader::readFile() {
	...
	{
		int posx, posy, id;
		int matchingWordsCount = sscanf(thisline.c_str(), "%d[fontsize=\"8\",pos=\"%d,%d\"];",&id, &posx, &posy); //set id posx and posy to matching strings
		...
		if (matchingWordsCount == 3) {
			...
			int weight, id2;
			matchingWordsCount = sscanf(thisline.c_str(), "%d--%d [fontsize=\"8\",label=\"%d\"];",&id, &id2, &weight);
			if (matchingWordsCount == 3) {
